...
\draw (1, -1.5) node[nmos](n_tail){};
\draw (3, -1.5) node[nmos, xscale=-1](p_tail){};
\draw (n_tail.source) to[short] (1, -2.5) to[short, -*] (2, -2.5) node[ground]{};
\draw (2, -2.5) to[short] (3, -2.5) to[short] (p_tail.source);
\draw (-1, -1.5) node[nmos, xscale=-1](bias){};
...
Since we know the pole location of the TIA, we can design the CTLE to have its zero in close proximity. Firstly, we draw the differential mode half circuit:
\begin{center}
PIc of HALF CIRcuit heRE
\end{center}
